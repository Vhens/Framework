<template>
  <section class="gt-notice">
    <ul class="roll-wrap f-cb" :style="{'transform' : 'translate3d('+translateX+'px,' + translateY + 'px, 0)','-webkit-transform' : 'translate3d('+translateX+'px,' + translateY + 'px, 0)'}"
      :class="{ 'isTransition' :  isTransition}">
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.type == "announcement") {
        this.baseList.unshift(this.baseList[this.baseList.length - 1]); 
        this.baseList.push(this.baseList[1]); 
      } else {
